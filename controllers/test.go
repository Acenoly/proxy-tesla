...
func AuthController1(c *gin.Context) {
	user := c.Query("user")
...
func TrafficController1(c *gin.Context) {
	...
	session := infos[3]
	itype := infos[4]
	fmt.Println(server_addr, client_addr, target_addr, username, bytes, user_username, country, level, session, itype)
